...
let newObjects = calculators.map(c => 
  `  { id: '${c.id}', title: '${c.title.replace(/'/g, "\\'")}', slug: '${c.id}', description: '${c.desc.replace(/'/g, "\\'")}', category: '${c.cat}', priority: 'P3', icon: 'Calculator', formula: '', howItWorks: [], faqs: [], countryTips: [], relatedSlugs: [], seoTitle: '', seoDescription: 'Free online ${c.title.replace(/'/g, "\\'")} Calculator. ${c.desc.replace(/'/g, "\\'")}.' }`
).join(',\n');
