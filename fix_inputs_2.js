...
const inputStart = content.indexOf('const INPUT_MAP');
const inputEnd = content.indexOf('export function', inputStart);
const beforePart = content.substring(0, inputStart);
let inputPart = content.substring(inputStart, inputEnd);
const afterPart = content.substring(inputEnd);

for (const [key, replacement] of Object.entries(repairs)) {
  const regex = new RegExp("'" + key + "': \\\\[[\\\\s\\\\S]*?\\\\](,?)\\n*", 'g');
  inputPart = inputPart.replace(regex, "'" + key + "': " + replacement + "$1\n");
}

fs.writeFileSync('src/pages/CalculatorDetail.tsx', beforePart + inputPart + afterPart);
console.log('Fixed inputs via slice replacement.');
